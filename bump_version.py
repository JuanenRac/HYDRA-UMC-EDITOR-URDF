#!/usr/bin/env python3
# =============================================================================
# HYDRA-UMC EDITOR-URDF - bump_version.py
# Copyright (C) 2026 JuanenRac (Electro Hobby 3D) <electrohobby3d@gmail.com>
# GPL-3.0 - see LICENSE
#
# Applies the ecosystem-wide "odometer" version bump to this project's own
# hydra_editor_urdf/__init__.py before every real build: PATCH goes up by 1;
# if that would push PATCH past 9, it resets to 0 and MINOR goes up by 1
# instead (e.g. 0.0.9 -> 0.1.0). MAJOR is never touched by this script - a
# deliberate manual-only decision, same convention across the ecosystem.
#
# Called from build_exe.bat/.sh right before PyInstaller runs, so every real
# packaged build carries a version 1 higher than the last real build. Also
# runs standalone (`python bump_version.py`), which is how it gets verified
# in isolation - no PySide6/PyInstaller import needed, just stdlib.
# =============================================================================
...
INIT_FILE = Path(__file__).resolve().parent / "hydra_editor_urdf" / "__init__.py"
...
def main() -> int:
    ...
    new_text = text[: match.start()] + f'__version__ = "{new_str}"' + text[match.end():]
