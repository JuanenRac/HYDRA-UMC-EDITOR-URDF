...
Traducción completa de la interfaz a través de **inglés, español, italiano, francés, alemán, chino simplificado y japonés** (`language/*.lng`), usando exactamente el mismo mecanismo de archivo plano `CLAVE=Valor` que cualquier otra herramienta Python de este ecosistema (URTC Flasher, URTC Tester, HYDRA-UMC SUITE) - no reinventado aquí, ya que el propio mecanismo no lleva ninguna lógica específica del proyecto. Un cambio de idioma surte efecto después de reiniciar la aplicación en vez de retraducir en vivo cada widget ya construido, siguiendo esa misma convención. `language/` está **junto a** el ejecutable en vez de empaquetado dentro de él vía el `--add-data` de PyInstaller, así que un traductor puede editar o añadir un archivo `.lng` sin necesidad de recompilar.

---

## 🎛️ Tema

La barra superior del espacio de trabajo acoplable es un panel de comandos
real con `QToolBar`/`QLabel`/`QToolButton`, no una UI Qt Quick/QML aparte -
una versión anterior integrada mediante QQuickWidget (mismo motor visual que
HYDRA-UMC-UPDATER y HYDRA-UMC-SUITE) se renderizaba como una barra negra
sólida sin ningún error en consola una vez colocada dentro del
`QDockWidget` real de este `QMainWindow`, así que se revirtió a widgets
planos; ver `CHANGELOG.md` para la historia completa. Sus botones Source,
DOF, Viewport, Properties y Upload sólo enfocan los docks existentes; Export
y About reusan las acciones existentes, y Export permanece deshabilitado
hasta que hay un modelo realmente cargado. Tras cargar un URDF (y tras cada
edición de propiedad en vivo), su chip de estado muestra el nombre del
modelo cargado, el conteo de DOF y el veredicto de viabilidad actual. No
sustituye el visor OpenGL, editor, parser ni la implementación de subida al
servidor.
...
├── requirements-dev.txt            # Solo numpy + pytest - lo que tests/ necesita de verdad, sin PySide6
...
├── tests/                          # Suite real de pytest (188 pruebas) - solo logica pura, sin ningun import de PySide6 aqui
│   ├── test_urdf_parser.py / test_urdf_writer.py  # URDF XML <-> models.py, incluyendo round-trips parse -> write -> parse
│   ├── test_dof.py                 # Validacion de viabilidad, limites de DOF, deteccion de raiz/huerfano/desconectado/multi-padre
│   ├── test_kinematics.py          # Cinematica directa, incluyendo la regresion ya arreglada del ciclo real que colgaba para siempre
│   ├── test_inertia_calc.py        # Tensores cerrados de caja/cilindro/esfera contra valores de manual calculados de forma independiente
│   ├── test_models.py              # Robot.root_link_name()/joints_by_parent()/movable_joints()
│   ├── test_scan.py                # find_urdf_files()/build_mesh_resolver(), incluyendo la regresion ya arreglada de had_scheme
│   ├── test_local_folder.py / test_github_fetcher.py  # Validacion de fuente; las llamadas de red a GitHub estan simuladas, nunca reales
│   ├── test_i18n.py                # E/S de config/idioma contra archivos temporales reales, nunca el config real del operador
│   └── test_gallery.py             # Comprobaciones estructurales sobre la lista GALLERY curada a mano
├── tools/
...
Encontrado al auditar el código: la lógica pura de esta app (parseo/exportación de URDF, validación de viabilidad DOF, cinemática directa, estimación de inercia, resolución de referencias de malla, obtención desde GitHub, i18n) tenía cero cobertura de pruebas automatizadas. Arreglado: 188 pruebas reales repartidas en 11 módulos de test, ninguno de los cuales importa PySide6/PyOpenGL - `requirements-dev.txt` solo instala lo que `tests/` necesita de verdad (`numpy` + `pytest`), así que la suite corre sin pantalla ni plugin de Qt. Varias pruebas son regresiones explícitas de errores ya encontrados y arreglados en una pasada de auditoría anterior y documentados directamente en el propio código fuente (la protección contra bucle infinito en un ciclo real de `render/kinematics.py`, la condición `had_scheme` de `source/scan.py`, la comprobación de masa negativa de `urdf/dof.py`) - una edición futura no puede reintroducir silenciosamente ninguno de ellos sin que falle una prueba. La CI (`.github/workflows/ci.yml`) ejecuta el mismo comando en cada push.
...
**Proyecto Padre**
- **[HYDRA-UMC-STUDIO](https://github.com/JuanenRac/HYDRA-UMC-STUDIO)** — el catálogo de modelos que este editor existe para poblar; el resultado terminado se envía directamente a un servidor STUDIO en marcha vía `POST /api/models/submit`.

**Directamente Relacionados**
- **[HYDRA-UMC-EDITOR-STL](https://github.com/JuanenRac/HYDRA-UMC-EDITOR-STL)** — editor de escritorio hermano para la geometría STL cruda del mismo catálogo de modelos, en vez del lado URDF/cinemática que edita esta herramienta.
- **[HYDRA-UMC-SERVER](https://github.com/JuanenRac/HYDRA-UMC-SERVER)** — posee el endpoint real `POST /api/models/submit` al que este editor envía los modelos terminados.
- **[HYDRA-UMC-TWIN](https://github.com/JuanenRac/HYDRA-UMC-TWIN)** — consume los modelos URDF creados aquí para impulsar su simulación física.
- **[HYDRA-UMC-PHYSICS-REPLICA](https://github.com/JuanenRac/HYDRA-UMC-PHYSICS-REPLICA)** — consume los modelos URDF creados aquí para impulsar su simulación física.
...
- **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** — la forma interna propia del editor: por qué el parseo, la validación de viabilidad, la resolución de mallas y la previsualización 3D son carriles separados, y lo que esta app deliberadamente **no** hace (conectarse a un robot, subir un URDF o mandar movimiento por su cuenta).
- **[docs/BUILD_AND_RUN.md](docs/BUILD_AND_RUN.md)** — la ruta de validación no destructiva `build-test.bat`/`.sh` frente a un paquete real `build_exe.bat`/`.sh`, y qué es realmente hoy el panel de comandos (`QToolBar`, no Qt Quick/QML - ver la sección **Tema** arriba).
- **[docs/INTEGRATION_CONTRACT.md](docs/INTEGRATION_CONTRACT.md)** — lo que un consumidor final de un archivo URDF exportado debe validar por su cuenta; este proyecto no ofrece ningún endpoint de red ni autoridad de control de hardware propia.
