...
from hydra_editor_urdf.models import Inertial, JointLimit, JointType, Material, MeshGeometry, Robot
...
class PropertiesPanel(QWidget):
    def __init__(self, controller: EditorController, parent=None):
        ...
        # Joint above, so an operator can eyeball or hand-tweak a
        # computed value before it's committed.
        self._inertial_group = QGroupBox(_("PROPS_INERTIAL_GROUP"))
        inertial_form = QFormLayout(self._inertial_group)

        self._inertial_mass = QDoubleSpinBox()
        self._inertial_mass.setRange(0.0, 1e6)
        self._inertial_mass.setDecimals(4)
        self._inertial_mass.setSuffix(" kg")
        inertial_form.addRow(_("PROPS_INERTIAL_MASS"), self._inertial_mass)

        self._inertial_ixx = QDoubleSpinBox()
        self._inertial_iyy = QDoubleSpinBox()
        self._inertial_izz = QDoubleSpinBox()
        for spin, label in ((self._inertial_ixx, "Ixx"), (self._inertial_iyy, "Iyy"), (self._inertial_izz, "Izz")):
            spin.setRange(0.0, 1e6)
            spin.setDecimals(8)
            inertial_form.addRow(label, spin)

        self._inertial_note = QLabel("")
        self._inertial_note.setWordWrap(True)
        inertial_form.addRow(self._inertial_note)

        self._inertial_calc = QPushButton(_("PROPS_CALC_INERTIAL"))
        self._inertial_calc.clicked.connect(self._on_calc_inertial)
        inertial_form.addRow(self._inertial_calc)

        self._inertial_apply = QPushButton(_("PROPS_APPLY_INERTIAL"))
        self._inertial_apply.clicked.connect(self._on_apply_inertial)
        inertial_form.addRow(self._inertial_apply)
        outer.addWidget(self._inertial_group)
        ...
        self._selected_visual_geometry = None
    ...
    def _set_editors_enabled(self, enabled: bool) -> None:
        ...
        self._inertial_group.setEnabled(enabled)
    ...
    def _on_selection_changed(self, link_name: str | None) -> None:
        ...
        self._selected_visual_geometry = first_visual.geometry if first_visual is not None else None
        self._inertial_group.setEnabled(first_visual is not None)
        self._inertial_note.setText("")
        existing = link.inertial
        self._inertial_mass.setValue(existing.mass if existing is not None else 0.0)
        self._inertial_ixx.setValue(existing.ixx if existing is not None else 0.0)
        self._inertial_iyy.setValue(existing.iyy if existing is not None else 0.0)
        self._inertial_izz.setValue(existing.izz if existing is not None else 0.0)
    ...
    def _on_calc_inertial(self) -> None:
        if self._selected_visual_geometry is None:
            return
        geometry = self._selected_visual_geometry
        mesh_bbox_size: tuple[float, float, float] | None = None
        if isinstance(geometry, MeshGeometry):
            # Resolve the real mesh file the same way the viewport already
            # does (controller.mesh_resolver), then load it to get real
            # vertex data for a bounding box - lazy import, this is the
            # one path in this panel that ever needs numpy/stl at all.
            resolved = self._controller.mesh_resolver(geometry.filename)
            if resolved is None:
                self._inertial_note.setText(_("PROPS_INERTIAL_MESH_UNRESOLVED"))
                return
            try:
                ...
                self._inertial_note.setText(_("PROPS_INERTIAL_MESH_LOAD_FAILED"))
                return

        known_mass = self._inertial_mass.value() or None
        result = estimate_inertial(geometry, known_mass=known_mass, mesh_bbox_size=mesh_bbox_size)
        if result is None:
            self._inertial_note.setText(_("PROPS_INERTIAL_MESH_UNRESOLVED"))
            return

        self._inertial_mass.setValue(result.mass)
        self._inertial_ixx.setValue(result.ixx)
        self._inertial_iyy.setValue(result.iyy)
        self._inertial_izz.setValue(result.izz)
        ...
        self._inertial_note.setText(" ".join(notes))

    def _on_apply_inertial(self) -> None:
        if self._robot is None or self._link_name is None:
            return
        link = self._robot.links[self._link_name]
        link.inertial = Inertial(
            mass=self._inertial_mass.value(),
            ixx=self._inertial_ixx.value(),
            iyy=self._inertial_iyy.value(),
            izz=self._inertial_izz.value(),
        )
        self._controller.notify_tree_changed()
